...
const TopMenu = (props) => {
  let external = (item) => {
      return (
          <li className="pure-menu-item">
              <a href={item.link} className="pure-menu-link">{item.label}</a>
          </li>
      );
  };
  let internal = (item, key) => {
      return (
  ...
  const items = props.items.map((item, key) => (
    item.external ? external(item) : internal(item, key)
  ));
...
export class App extends Component {
  ...
  render() {
    ...
    const menuItems = [
        ...
        {label: 'See api (swagger)', link: '/swagger-ui.html', external: true},
